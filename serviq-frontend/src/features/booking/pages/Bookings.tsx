...
import {
  ...
  getCustomerBookingsAPICall,
  getWorkerBookingsAPICall,
...
const Bookings = () => {
  ...
  // Generate unique IDs for form fields to associate labels
  const uidBookingDate = useId();
  const uidBookingTime = useId();
  const uidCustomerPhone = useId();
  const uidCustomerAddress = useId();
  const uidProblemDesc = useId();
  const uidPrice = useId();
            ...
            await createProfileAPI(
              {
                ...
                profileImage: `https://api.dicebear.com/10.x/initials/svg?seed=${encodeURIComponent(
  ...
  const bookingStats = useMemo(
    () => ({
      ...
      active: bookings.filter((booking) =>
        ["Accepted", "In Progress"].includes(booking.bookingStatus),
      ).length,
  ...
  const submitBooking: SubmitHandler<BookingFormValues> = async (values) => {
    ...
    setConfirmationMessage(
      `Booking request submitted for ${values.bookingDate} at ${values.bookingTime}. We’ll assign the best available professional shortly.`,
    );
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="overflow-hidden rounded-[1.25rem] border border-gray-200 bg-white shadow-[0_8px_30px_rgba(0,0,0,0.04)]">
        {/* Header */}
        <div className="relative overflow-hidden bg-gradient-to-br from-gray-900 via-gray-800 to-gray-700 px-6 py-8 text-white sm:px-8 lg:px-10">
          <div
          ...
          <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.28em] text-gray-300">
                {isWorker ? "Worker bookings" : "Bookings"}
              </p>
            ...
            <Link
              to="/dashboard"
              className="inline-flex items-center gap-2 self-start rounded-full border border-white/20 bg-white/10 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-white/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-gray-800 active:scale-[0.98]"
            >
        ...
        <div className="p-6 sm:p-8">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              <div
                key={stat.label}
                className="rounded-xl border border-gray-200 bg-white px-4 py-4 shadow-sm transition hover:shadow-md"
              >
          ...
          <div className="mt-8 grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
            {/* Left panel – Booking form (customer) or Worker overview */}
            {!isWorker ? (
              <section className="rounded-[1.25rem] border border-gray-200 bg-white p-5 sm:p-6">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h2 className="text-xl font-bold text-black">Book a service</h2>
                    <p className="mt-1 text-sm text-gray-500">
                      Choose a category, pick a date and time, and let our system assign the right
                      professional.
                    </p>
                  </div>
                ...
                {/* Visual card */}
                <div className="mt-5 overflow-hidden rounded-xl border border-gray-200 bg-gray-900 text-white shadow-sm">
                  <img
                  ...
                  <div className="p-4 sm:p-5">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-gray-200">
                        Fast booking
                      </span>
                      <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-gray-200">
                        Verified help
                    ...
                    <p className="mt-3 text-sm leading-6 text-gray-300">
                      Your request is matched with the best nearby expert in minutes, with clear
                      pricing and trusted support.
                    </p>
                  </div>
                </div>
                ...
                <form onSubmit={handleSubmit(submitBooking)} className="mt-6 space-y-6">
                  ...
                  {confirmationMessage ? (
                    <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-4 text-sm text-emerald-700 shadow-sm">
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                          ...
                          {confirmationDetails?.assignedWorkerName ? (
                            <p className="mt-2 font-semibold text-emerald-800">
                              Assigned professional: {confirmationDetails.assignedWorkerName}
                            </p>
                          ) : (
                            <p className="mt-2">
                              The system is matching you with the best available professional.
                            </p>
                          )}
                        ...
                        {confirmationDetails?.bookingId ? (
                          <button
                            type="button"
                            onClick={() =>
                              navigate(`/bookings/${confirmationDetails.bookingId}`)
                            }
                            className="rounded-full bg-black px-4 py-2 text-sm font-semibold text-white shadow transition hover:bg-gray-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-2 active:scale-[0.98]"
                          >
                            View details
                          </button>
                        ) : null}
                        ...
                        <button
                          ...
                          className={`group rounded-xl border px-4 py-4 text-left transition-all duration-300 ${
                            selectedCategory === category.label
                              ...
                              : "border-gray-200 bg-white text-gray-900 hover:-translate-y-1 hover:border-black hover:shadow-md"
                  ...
                  {selectedCategory ? (
                    <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-700">
                      We'll automatically assign the best available worker for{" "}
                      <span className="font-semibold">{selectedCategory}</span> based on your chosen
                      date and time.
                    </div>
                  ) : null}
                  ...
                  <div className="rounded-xl border border-gray-200 bg-white p-5">
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      Share your preferred time, address, and contact details.
                    </p>

                    <div className="mt-4 grid gap-4 sm:grid-cols-2">
                      <Field label="Booking Date" error={errors.bookingDate?.message} inputId={uidBookingDate}>
                        <input
                          id={uidBookingDate}
                          type="date"
                          min={today}
                          {...register("bookingDate")}
                          className="input rounded-xl border-gray-300 focus:border-black focus:ring-2 focus:ring-black/10"
                        />
                      </Field>
                      <Field label="Booking Time" error={errors.bookingTime?.message} inputId={uidBookingTime}>
                        <select
                          id={uidBookingTime}
                      ...
                      <Field label="Phone" error={errors.customerPhoneNumber?.message} inputId={uidCustomerPhone}>
                        <input
                          id={uidCustomerPhone}
                    ...
                    <div className="mt-4">
                      <Field label="Service address" error={errors.customerAddress?.message} inputId={uidCustomerAddress}>
                        <textarea
                          id={uidCustomerAddress}
                          rows={3}
                    ...
                    <div className="mt-4">
                      <Field label="Problem Description" error={errors.problemDescription?.message} inputId={uidProblemDesc}>
                        <textarea
                          id={uidProblemDesc}
                          rows={4}
                    ...
                    <div className="mt-4">
                      <Field label="Price" error={errors.price?.message} inputId={uidPrice}>
                        <input
                          id={uidPrice}
                          type="number"
                          min="1"
                  ...
                  <button
                    ...
                    className="w-full rounded-full bg-black px-6 py-3.5 text-base font-bold text-white shadow-md transition hover:bg-gray-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-2 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    {isSubmitting ? (
                      <span className="flex items-center justify-center gap-2" role="status" aria-label="Creating booking">
              ...
              <section className="rounded-[1.25rem] border border-gray-200 bg-white p-5 sm:p-6">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h2 className="text-xl font-bold text-black">Worker overview</h2>
                    <p className="mt-1 text-sm text-gray-500">
                      Only assigned job details are shown for workers.
                    </p>
                  </div>
                  <span className="rounded-full bg-gray-100 px-3 py-1 text-xs font-semibold text-gray-600">
                    Worker view
                ...
                <div className="mt-6 rounded-xl border border-gray-200 bg-gray-50 p-5">
                  <h3 className="text-base font-semibold text-black">What you can do</h3>
                  <ul className="mt-3 space-y-2 text-sm leading-6 text-gray-600">
                    ...
                    <li>
                      See customer address, contact numbers, and service description only when
                      needed.
                    </li>
                  </ul>
                </div>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h2 className="text-xl font-bold text-black">
                      {isWorker ? "Assigned bookings" : "My bookings"}
                    </h2>
                    <p className="mt-1 text-sm text-gray-500">
                      Recent requests and service progress from the backend.
                    </p>
                  </div>
                  ...
                  {bookingsLoading ? (
                    ...
                    bookings.slice(0, 5).map((booking) => (
                      <Link
                        ...
                        className="group block rounded-xl border border-gray-200 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:border-gray-300 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-2"
                      >
                        ...
                        <p className="mt-3 text-sm leading-6 text-gray-600">
                          {isWorker ? booking.customerAddress : booking.problemDescription}
                        </p>
                    ...
                    <div className="flex flex-col items-center rounded-xl border border-dashed border-gray-300 bg-gray-50 px-4 py-10 text-center">
                      <svg className="mb-3 h-10 w-10 text-gray-400" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" aria-hidden="true">
...
const DetailPill = ({ label, value }: { label: string; value: string }) => (
  <div className="rounded-xl border border-gray-200 bg-gray-50 px-4 py-3">
    ...
    <p className="mt-1.5 text-sm font-semibold text-black">{value}</p>
  </div>
);
